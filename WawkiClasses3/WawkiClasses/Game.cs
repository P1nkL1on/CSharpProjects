...
namespace WawkiClasses
{
    ...
    public class Game : GameBotView
    {
                    ...
                    {//stepping from this point you have almost 1 kill
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            ...
                            for (int step = 1; step < 8; step++)
                            {
                                ...
                                if (podhod == 1 && pregrada != null
                                    && newPath.IndexOf(pregrada.Position) < 0)
        ...
        public string CanBePlayed()
        {
            ...
            for (int i=0; i<currentField.FigureCount; i++)
                wins[currentField.GetFigureBy(i).Team -1]++;
            if (wins[0] == 0) return "Player 2 wins!";
            if (wins[1] == 0) return "Player 1 wins!";
            return "";
        }
